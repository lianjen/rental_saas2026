    ...
    if ledger_df is None or ledger_df.empty:
        ...
        for col in ["預收電費", "扣電費", "餘款"]:
            if col in ledger_display.columns:
                ledger_display[col] = ledger_display[col].apply(
        ...
        col_sel, col_btn, col_hint = st.columns([4, 1, 2])
        with col_sel:
            selected_delete_label = st.selectbox(
                "選擇要刪除的記錄",
下面是完整可覆蓋版 `views/electricity.py`，以你目前的 v4.9 檔案為基底，保留原本 4 個 tabs，並新增第 5 個「💰 電費預收帳」tab。[cite:311]
...
✅ [NEW v5.0] 新增 Tab 5「💰 電費預收帳」
✅ [NEW v5.0] 全房間餘款總覽
✅ [NEW v5.0] 房間流水帳查詢 / 刪除錯誤記錄
✅ [NEW v5.0] 新增預收 / 扣電費（可關聯 period_id）
"""
...
def _get_selected_period_default_index(period_options: Dict[str, Optional[int]], default_period_id: Optional[int]) -> int:
...
def render_records_tab(elec_service: ElectricityService):
    ...
    col_toggle, col_hint = st.columns()
    with col_toggle:
        hide_1f = st.toggle(
            "🙈 隱藏 1F (1A/1B)",
            value=False,
...
def render_statistics_tab(elec_service: ElectricityService):
    ...
    filtered = (
        stats_df.copy()
        if selected_year == "全部"
        else stats_df[stats_df["年份"] == int(selected_year)].copy()
    )
    ...
    with st.expander("🏠 展開各房間用電趨勢", expanded=False):
        ...
        rt = (
            room_trend_df.copy()
            if selected_year == "全部"
            else room_trend_df[room_trend_df["年份"] == int(selected_year)].copy()
        )
...
def render_deposit_tab(elec_service: ElectricityService):
    ...
    if summary_df is None or summary_df.empty:
        st.info("目前尚無任何預收電費資料")
    else:
        display_summary = summary_df.copy()

        for col in ["預收總額", "扣除總額", "當前餘款"]:
            if col in display_summary.columns:
                display_summary[col] = display_summary[col].apply(_to_int_safe)

        total_credit  = _to_int_safe(display_summary["預收總額"].sum()) if "預收總額" in display_summary.columns else 0
        total_debit   = _to_int_safe(display_summary["扣除總額"].sum()) if "扣除總額" in display_summary.columns else 0
        total_balance = _to_int_safe(display_summary["當前餘款"].sum()) if "當前餘款" in display_summary.columns else 0

        c1, c2, c3 = st.columns(3)
        with c1:
            metric_card("預收總額", f"${total_credit:,}", "", "💵", "normal")
        with c2:
            metric_card("累計扣除", f"${total_debit:,}", "", "⚡", "warning")
        with c3:
            metric_card("總餘款", f"${total_balance:,}", "", "🏦", "success")

        def _balance_flag(v) -> str:
            amount = _to_int_safe(v)
            if amount < 0:
                return "🔴"
            if amount <= 500:
                return "🟠"
        ...
        display_summary["狀態"] = display_summary["當前餘款"].apply(_balance_flag)
        display_summary = display_summary[
            ["狀態", "房號", "租客", "預收總額", "扣除總額", "當前餘款", "最近一筆"]
        ].sort_values(["當前餘款", "房號"], ascending=[True, True]).reset_index(drop=True)
    ...
    default_room_index = room_options.index("2A") if "2A" in room_options else 0
    selected_room = st.selectbox(
        "選擇房號",
        options=room_options,
    ...
    b1, b2 = st.columns()
    with b1:
        balance_color = "success" if current_balance > 500 else ("warning" if current_balance >= 0 else "error")
        metric_card("目前餘款", f"${_to_int_safe(current_balance):,}", "", "💰", balance_color)
    with b2:
        if current_balance < 0:
            st.error("⚠️ 目前餘款為負，代表已先扣後補")
        elif current_balance <= 500:
            st.warning("⚠️ 餘款偏低，建議提醒租客補預收")
        else:
            st.success("✅ 餘款充足")
    ...
    if ledger_df is None or ledger_df.empty:
        ...
        ordered_cols = ["id", "日期", "類型", "說明", "預收電費", "扣電費", "餘款", "期間ID"]
        ledger_display = ledger_display[[c for c in ordered_cols if c in ledger_display.columns]]
