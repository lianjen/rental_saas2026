...
✅ 統一金額類型轉換
✅ 租金追蹤 + 電費追蹤 + 綜合追蹤
✅ [FIX v3.1] use_container_width → width (共 13 處)
"""
...
def render_rent_tracking(payment_service: PaymentService, tenant_service: TenantService):
    ...
    try:
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("待繳款", f"{len(df[df['status'] == 'unpaid'])} 筆")
        with col2:
...
def render_electricity_tracking(
    ...
    try:
        ...
        with st.spinner("正在載入電費記錄..."):
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("待繳款", f"{len(df[df['繳費狀態']=='⏳ 未繳'])} 筆")
            with col2:
                st.metric("已繳", f"{len(df[df['繳費狀態']=='✅ 已繳'])} 筆")
            with col3:
                st.metric("應收總額", f"${df['應繳金額_數值'].sum():,.0f}")
            with col4:
                st.metric("已收金額", f"${df['已繳金額_數值'].sum():,.0f}")
...
def render_combined_tracking(
    ...
    with col3:
        st.metric(
            "💵 總待收金額",
            f"${float(rent_total) + float(elec_total):,.0f}",
            delta=f"{rent_count + elec_count} 筆",
        )
    with col4:
        st.metric("📊 收繳概況", f"{rent_count + elec_count} 筆待繳")
    ...
    with col_rent:
        ...
        if not rent_df.empty:
            ...
            display_df["payment_period"] = display_df.apply(
                lambda row: f"{row['payment_year']}/{row['payment_month']:02d}", axis=1
            )
