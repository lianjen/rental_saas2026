...
✅ 支出列表（含編輯/刪除）
✅ 統計分析（月度/年度/類別）
✅ [FIX v2.1] use_container_width → width (共 5 處)
✅ [FIX v2.1] confirm-without-desc 移出 form，修正永不顯示問題
"""
...
try:
    ...
    def data_table(df, key="table"):
        st.dataframe(df, width="stretch", key=key)   # ✅ FIX 1
...
def render_add_tab(expense_service: ExpenseService):
    ...
    # ✅ FIX 6: confirm 提示移到 form 外，讓它在 rerun 後正常顯示
    ...
    with st.form("add_expense_form"):
        ...
        with col2:
            amount = st.number_input(
                "金額", min_value=0, value=0, step=100, key="add_amount"
            )
        ...
        if submitted:
            if amount <= 0:
                ...
                # ✅ FIX 6: 暫存資料到 session_state，form 外顯示確認
                ...
                ok, msg = expense_service.add_expense(
                    expense_date.isoformat(), category, amount, description
                )
                if ok:
                    st.success("✅ 新增成功")
                    st.balloons()
...
def render_list_tab(expense_service: ExpenseService):
    ...
    if "id" in df.columns and len(df) > 0:
        ...
        if selected_label != "-- 請選擇 --":
                ...
                with st.expander("✏️ 編輯支出", expanded=True):
                    with st.form("edit_expense_form"):
                        ...
                        with cole1:
                            edit_category = st.selectbox(
                                ...
                                index=(
                                    ...
                                    if expense_row["category"] in EXPENSE.CATEGORIES
                                    else 0
                                ),
                                key="edit_category",
                            )
                        ...
                        if st.form_submit_button(
                            "💾 儲存變更", type="primary", width="stretch"
                        ):
            ...
            with col_delete:
                ...
                st.write("")
                if st.button(
                    "🗑️ 刪除", type="secondary", key="delete_btn", width="stretch"
                ):
    ...
    cols_to_show = [
        col
        for col in ["id", "expense_date", "category", "amount_display", "description"]
...
def render_stats_tab(expense_service: ExpenseService):
    ...
    if stats_type == "月度分析":
        month = st.selectbox(
            "月份", range(1, 13), index=(date.today().month - 1), key="stats_month"
        )
        ...
        st.divider()
        st.write("**明細**")
        category_display = category_sum.copy()
        category_display["金額"] = category_display["金額"].apply(lambda x: f"${x:,.0f}")
        st.dataframe(
            category_display,
            width="stretch",      # ✅ FIX 3
            hide_index=True,
    ...
    # ===== 類別分析 =====
        ...
        category_stats["佔比"] = (
            category_stats["總金額"] / total_year * 100
        ).round(1)
