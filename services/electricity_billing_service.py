"""
電費帳務服務 - v5.6
✅ 電費 DB/service 實作搬移到 electricity_billing_service.py
✅ 保留既有快取與寫入後清 cache 行為
✅ 供 electricity_service.py facade 向下相容使用
"""
...
def _cached_get_taipower_bills(
    ...
    return ElectricityBillingService()._get_taipower_bills_uncached(period_id)
...
def _cached_get_deposit_ledger(
    ...
    return ElectricityBillingService()._get_deposit_ledger_uncached(room_number)
...
def _cached_get_all_rooms_deposit_summary(
    ...
    return ElectricityBillingService()._get_all_rooms_deposit_summary_uncached()


@cache_data(ttl=600)
def _cached_get_all_periods(user_id: str, dev_mode: bool) -> List[Dict]:
    return ElectricityBillingService()._get_all_periods_uncached()
...
def _cached_get_all_readings(
    ...
    return ElectricityBillingService()._get_all_readings_uncached(period_id)
...
def _cached_get_payment_record(
    ...
    return ElectricityBillingService()._get_payment_record_uncached(period_id)
...
class ElectricityBillingService(BaseDBService):
